// Based on http://www.jonolick.com/code.html (public domain)

// Glitches an image by running it through a broken jpeg encoder
// Usage: glitch INFILE OUTFILE
...
unsigned char s_jo_ZigZag[] = { 0,1,5,6,14,15,27,28,2,4,7,13,16,26,29,42,5,8,12,17,25,30,41,43,9,11,18,24,31,40,44,53,10,19,23,32,39,45,52,54,20,22,33,38,46,51,55,60,21,34,37,47,50,56,59,61,35,36,48,49,57,58,62,63 };

void glitch_zigzag(unsigned char *zigzag, int minchanges, int maxchanges) {
    int good_indices[] = {46, 49};

    int changes = minchanges;
    if(maxchanges - minchanges > 0)
        changes += random() % (maxchanges + 1 - minchanges);
    
    int i;
    for(i = 0; i < changes; i ++) {
        int index = good_indices[random() % (sizeof(good_indices) / sizeof(int))];
        zigzag[index] = random() % 63;
    }
}

int glitch_std_ac_values(unsigned char *values, int minchanges, int maxchanges) {
    int good_indices[] = {6, 7, 8, 11, 12, 16, 17, 18, 21, 32, 38};
    ...
    for(i = 0; i < changes; i ++) {
        int index = good_indices[random() % (sizeof(good_indices) / sizeof(int))];
        values[index] = random() % 0xff;
    }

    return changes;
}

int glitch(void *array_void, size_t array_size, size_t item_length,
            int minswaps, int maxswaps) {
...
bool jo_write_jpg(const char *filename, const void *data, int width, int height, int comp, int quality) {
    ...
    if(glitches < max_glitches)
        glitches += glitch_std_ac_values(std_ac_luminance_values, 0, 1);
    ...
    if(glitches < max_glitches)
        glitches += glitch_std_ac_values(std_ac_chrominance_values, 0, 1);
    
    // Huffman tables
    unsigned short YDC_HT[256][2] = { {0,2},{2,3},{3,3},{4,3},{5,3},{6,3},{14,4},{30,5},{62,6},{126,7},{254,8},{510,9}};
    if(glitches < max_glitches)
        glitches += glitch(YDC_HT, sizeof(YDC_HT), sizeof(unsigned short), 0, 1);

    unsigned short UVDC_HT[256][2] = { {0,2},{1,2},{2,2},{6,3},{14,4},{30,5},{62,6},{126,7},{254,8},{510,9},{1022,10},{2046,11}};
    if(glitches < max_glitches)
        glitches += glitch(UVDC_HT, sizeof(UVDC_HT), sizeof(unsigned short), 0, 1);
    ...
    if(glitches < max_glitches)
        glitches += glitch(UVAC_HT, sizeof(UVAC_HT), sizeof(unsigned short), 1, 1);
